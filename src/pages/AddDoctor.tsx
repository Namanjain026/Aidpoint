...
const AddDoctor = ({ doctorEmail }: AddDoctorProps) => {
  const doctorContext = useContext(DoctorContext);
  if (!doctorContext) {
    throw new Error("DoctorContext must be used within a DoctorProvider");
  }
  const { addDoctor, fetchDoctors } = doctorContext;
  ...
  const [isSubmitted, setIsSubmitted] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [isEditing, setIsEditing] = useState<boolean>(!!doctorEmail);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Refresh DoctorContext to sync with Supabase
      await fetchDoctors();
      ...
      if (err.code === "23505") {
        ...
        setError("Permission denied. Please check your authentication or database policies.");
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-purple-100 p-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8 pt-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800 mb-2">{isEditing ? "Edit Doctor" : "Add New Doctor"}</h1>
          <p className="text-gray-600">{isEditing ? "Update doctor details" : "Join our healthcare network"}</p>
        </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      disabled={isEditing}
                      className="w-full px-4 py-3 border border-gray-200 bg-gray-50 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all text-black disabled:bg-gray-100 disabled:cursor-not-allowed"
                      placeholder="doctor@hospital.com"
                    />
                  </div>
                </div>
