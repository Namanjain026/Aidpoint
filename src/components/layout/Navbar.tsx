...
//   // Updated navigation array with Hospitals link
//   const navigation = [
//     { name: "Home", href: "/" },
//     { name: "Profile", href: "/Patient-Dashboard" },
//     // { name: "Hospital", href: "/HospitalDashboard" },
//     { name: "Hospitals", href: "/hospital-search" }, // Added Hospitals link
//     ...(isAuthenticated
//       ? [{ name: "Patient Record", href: "/appointments" }]
//       : []),
//     ...(user?.role === "hospital"
//       ? [{ name: "Dashboard", href: "/dashboard" }]
//       : []),
...
const Navbar = () => {
  ...
  // Handle Patient Record click
  const handlePatientRecordClick = (e: React.MouseEvent) => {
    e.preventDefault();
  ...
  const navigation = [
    ...
    ...(isAuthenticated
      ? [{ name: "Patient Record", href: "/appointments", onClick: handlePatientRecordClick }]
      : []),
    ...(user?.role === "hospital"
      ? [{ name: "Dashboard", href: "/dashboard" }]
      : []),
            ...
            {navigation.map((item) => (
              item.onClick ? (
                <button
                  key={item.name}
                  onClick={item.onClick}
                  className={`w-full text-left block px-4 py-3 rounded-xl text-base font-medium transition-all duration-200 ${
                    isActive(item.href)
                      ? "text-white bg-gradient-to-r from-blue-600 to-purple-600 shadow-lg"
                      : "text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-gray-800"
                  }`}
